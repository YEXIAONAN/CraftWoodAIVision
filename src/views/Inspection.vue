    ...
    <el-row :gutter="20">
      ...
      <el-col :span="10">
        <el-card shadow="never">
          <template #header>上传检测图片</template>
          <div
            class="upload-area"
          ...
          >
            ...
            <el-icon :size="48" color="#B8845E"><UploadFilled /></el-icon>
          ...
          <div v-if="preview" class="preview-wrap">
            <el-image :src="preview" fit="contain" style="width:100%;height:240px;border-radius:8px" />
            <div class="preview-actions" style="margin-top:12px;display:flex;gap:8px">
              ...
              <el-button type="primary" :loading="detecting" @click="startDetection">开始检测</el-button>
            </div>
          </div>
        </el-card>
      </el-col>
        ...
        <el-card shadow="never">
          <template #header>
            <div class="card-header">
              <span>检测结果</span>
              <span v-if="result" class="result-time">检测耗时: {{ result.duration }}ms</span>
            </div>
          </template>
          <div v-if="!result" class="empty-result">
            <el-icon :size="48" color="#D4A574"><Camera /></el-icon>
          ...
          <div v-else>
            <div class="result-summary">
              ...
              <div class="summary-item">
                ...
                <span :class="['score', { 'score-high': result.score >= 85, 'score-medium': result.score >= 70 && result.score < 85, 'score-low': result.score < 70 }]">{{ result.score }}</span>
              </div>
              <div class="summary-item">
            ...
            <el-divider />
            <h4 style="margin-bottom:12px">缺陷明细</h4>
            <div class="defect-list">
              <el-tag v-for="(d, i) in result.defects" :key="i" closable :type="d.confidence > 0.85 ? 'danger' : 'warning'" class="defect-item" @close="result.defects.splice(i, 1)">
...
<style scoped>
.inspection-page { max-width: 1200px; margin: 0 auto; }
...
.upload-hint { margin-top: 4px; font-size: 12px; color: var(--color-text-secondary); }
.empty-result { text-align: center; padding: 40px; color: var(--color-text-secondary); }
.empty-result p { margin-top: 12px; }
.result-summary { display: flex; gap: 32px; }
.summary-item { display: flex; flex-direction: column; gap: 4px; }
.summary-item .label { font-size: 12px; color: var(--color-text-secondary); }
.score { font-size: 24px; font-weight: 700; }
.score-high { color: #67c23a; }
.score-medium { color: #e6a23c; }
.score-low { color: #f56c6c; }
.defect-count { font-size: 24px; font-weight: 700; color: var(--color-primary); }
.defect-list { display: flex; flex-wrap: wrap; gap: 6px; }
.defect-item { font-size: 13px; }
.result-time { font-size: 12px; color: var(--color-text-secondary); }
